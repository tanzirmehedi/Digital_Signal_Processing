...
%w(n)= 0.50-.5cos(2*pi*n/N-1), 0<=n<=N-1
...
cutt_of_frequency = input('Input the cut off frequency: ') ;  % pi/3

range=cutt_of_frequency/(2*pi);

Window = rectwin(N+1); % hamming computes a length N+1 window and returns the first N points.
